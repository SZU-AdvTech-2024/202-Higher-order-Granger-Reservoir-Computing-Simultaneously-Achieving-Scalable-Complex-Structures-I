...
if ifadj:
    ...
    ho = '7-7-124'#[1, 7], [2, 7], [4, 1], [4, 2], [4, 4]
    edges_in = gen_ein(ho)
...
#动态导入模块：import models.Model_“model_ind” as mod

model = mod.model
    
ntr = int(args.T*args.qtr)#计算训练数据的样本数：总时间步长*训练数据的比例
experiment = model(args)#初始化模型
experiment.train()#训练
...
nj = 0
Vj = 0
preds,error = experiment.evalue1()#返回预测值和误差
...
#start2s 从 args.warm_up 到 ntr-steps 之间均匀分布 num+2 个点，并去掉头尾两个点（通过 [1:-1]），然后转为整数索引。
...
# Saving one-step prediction results 未调用
def sav_er1():
    er = np.zeros((2,3))
...
def sav_er2(): 
    ...
    for ni in range(args.n):#5
        for vj in range(args.V):#3
