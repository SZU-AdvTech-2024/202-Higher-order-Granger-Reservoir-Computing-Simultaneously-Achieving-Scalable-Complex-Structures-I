...
class data():
    ...
    def gen_net(self):
        args = self.args
        if args.direc:
            net = nx.DiGraph()  # 如果是有向图，使用 DiGraph
        else:
            net = nx.Graph()  # 如果是无向图，使用 Graph
        net.add_nodes_from(np.arange(self.n))  # 添加网络节点

        if args.net_nam == 'er':  # 如果网络类型是“er”，生成 Erdos-Renyi 随机图
            for u, v in nx.erdos_renyi_graph(self.n, 0.6).edges():
                net.add_edge(u, v, weight=random.uniform(0, 1))
        elif args.net_nam == 'ba':  # 如果网络类型是“ba”，生成 Barabási-Albert 无标度网络
            for u, v in nx.barabasi_albert_graph(self.n, 4).edges():
                net.add_edge(u, v, weight=random.uniform(1, 1))
        elif args.net_nam == 'rg':  # 如果网络类型是“rg”，生成随机正则图
            for u, v in nx.random_regular_graph(4, self.n).edges():
                net.add_edge(u, v, weight=random.uniform(1, 1))
        elif args.net_nam == 'edges':  # 如果网络类型是“edges”，使用预定义的边和权重
            ...
            print(args.net_nam)
        else:  # 如果网络类型是默认类型，从CSV文件加载边和权重数据
