...
def args(): 
    ...
    parser.add_argument('--data_ind', type=str, default='L96') 
    ...
    parser.add_argument('--ob_noise', type=float, default=0.0001)
    ...
    parser.add_argument('--input_scaling', type=float, default=0.06) 
    parser.add_argument('--noise_level', type=float, default=0.00) 
    parser.add_argument('--alpha', type=float, default=10**(-9))
...
if ifadj:
    ...
    def gen_inf(hon):
        ...
        for vi in range(args.V):
            if vi!=0:
                dec = 2**g[vi]
                edges_in.append([dec,dec])
                edges_in.append([dec,2**g[vi-1]+2**g[vi+1-args.V]])
                edges_in.append([dec,2**g[vi-1]+2**g[vi-2]])
        vi = 0
        dec = 2**g[vi]
        ho = ''
        for i in hon:
            edges_in.append([dec,i])
            if ho=='':
                ho = ho+str(i)
            else:
                ho = ho+'-'+str(i)
