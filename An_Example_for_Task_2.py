...
# 假设 lens3s 已经包含了VPS数据，这里进行缩放，单位为1000
lens3s_scaled = lens3s / 1000  # 将VPS的单位转为1000

# 绘制箱型图
plt.figure(figsize=(10,6))
box = plt.boxplot(lens3s_scaled.T, labels=methods, patch_artist=True, 
                  medianprops=dict(color='orange', linewidth=3), showcaps=True)  # 将showcaps设置为True

# 设置每个箱体的颜色，并且边缘颜色与箱体一致
...
# 设置轴标签和标题
plt.ylabel('VPS (in 1000)', fontsize=14)
plt.title(f'VPS Comparison Across Methods CL', fontsize=16)
...
# 保存图像为指定文件路径
plt.savefig(f'results/CL_vps_comparison_boxplot.png', dpi=300)  # 根据参数创建图像文件名
