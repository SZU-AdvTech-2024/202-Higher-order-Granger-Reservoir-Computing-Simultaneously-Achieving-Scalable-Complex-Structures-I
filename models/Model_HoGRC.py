...
class model():
    ...
    def read_data(self):
        args = self.args
        Xs = pd.read_csv("./dataset/data/trajectory.csv").values[:,1:].transpose()#不读取第一列（通常为时间步长），转置
        Xss = np.zeros((args.n, args.N*args.T, args.V))#网络规模，实验次数*时间步长，维度
        #存储每个样本的每个时间步的 V 个变量的数据。
        Xsn = np.zeros((args.N, args.n, args.T, args.V))
        #这个矩阵用于存储每个实验次数（N）、每个样本（n）、每个时间步长（T）的 V 个变量的数据。
    ...
    def VPT(self,error2s,error3s,steps,num):
        ...
        print(lens2)
        print(lens3)
        lens2_flat = lens2.flatten()  # 拉直为一列
        lens3_flat = lens3.flatten()  # 拉直为一列
    
        # 将它们转换为DataFrame
        lens2pd = pd.DataFrame(lens2_flat, columns=['lens2'])
        lens3pd = pd.DataFrame(lens3_flat, columns=['lens3'])
        
        # Save the data to CSV files with the appropriate naming format
        lens2pd.to_csv(f'results/HoGRC-HR_lens2.csv')
        lens3pd.to_csv(f'results/HoGRC-HR_lens3.csv')
        plt.figure(figsize=(12,6))
        plt.boxplot([lens2.flatten(), lens3.flatten()], labels=['Lens2 (Two-step)', 'Lens3 (Three-step)'])
        plt.ylabel('Prediction Time Steps')
        plt.title('VPS Comparison: Lens2 vs Lens3')
        plt.savefig('results/vps_comparison_boxplot.png')
        plt.show()
        return(lens2,lens3)
    ...
    def draw1(self,preds,preds2,preds3,start2,start3,steps,nj):
        ...
        for dimension in range(args.V):
            ...
            # 保存图形
            plt.show
            plt.savefig('results/multi_step_prediction.png')
    ...
    def draw2(self,preds,preds2,preds3,start2,start3,steps,nj):
        ...
        plt.savefig('results/multi_step_prediction-draw2.png')
    ...
    def draw_FHN(self,preds,preds2,preds3,start2,start3,steps,nj):    
        ...
        plt.savefig('results/multi_step_prediction-draw_FHN.png')
    ...
    def draw_3d(self,preds,preds2,preds3,start2,start3,steps,nj):    
        ...
        ax1.legend(prop=font1)
        plt.show()
        plt.savefig('results/multi_step_prediction-draw-3d.png')
    ...
    def draw3(self):
        ...
        ax1.legend(prop=font1)
        plt.savefig('results/multi_step_prediction-draw3.png')
