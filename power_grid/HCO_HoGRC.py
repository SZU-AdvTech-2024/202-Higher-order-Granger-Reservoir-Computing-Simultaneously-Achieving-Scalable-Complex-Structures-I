...
def draw1(args,preds,preds2,preds3,start2,start3,steps,nj):
    ...
    if args.V>2:
        ...
        # ax1.legend(prop=font1)  
    plt.savefig('power_grid/results/us.png', dpi=300, bbox_inches='tight')
...
def save_vps_to_csv(lens2, lens3, method_name, save_dir='results'):
    """
    将lens2和lens3数据拉直为一维并保存为CSV文件

    参数:
    lens2 (ndarray): 五维数组，需要保存为CSV的VPS数据
    lens3 (ndarray): 五维数组，需要保存为CSV的VPS数据
    method_name (str): 方法名称，将作为文件名的一部分
    save_dir (str): 保存文件的文件夹路径
    """
    
    # 拉直为一维
    lens2_flat = lens2.reshape(-1)  # 拉直lens2
    lens3_flat = lens3.reshape(-1)  # 拉直lens3

    # 打印展平后的形状以确保正确
    print(f"lens2_flat shape: {lens2_flat.shape}")
    print(f"lens3_flat shape: {lens3_flat.shape}")

    # 保存为 CSV 文件
    lens2_df = pd.DataFrame(lens2_flat)
    lens3_df = pd.DataFrame(lens3_flat)

    # 创建文件名，包含方法名
    lens2_filename = f"{save_dir}/VPS_{method_name}_lens2.csv"
    lens3_filename = f"{save_dir}/VPS_{method_name}_lens3.csv"

    # 将数据保存为CSV文件
    lens2_df.to_csv(lens2_filename, index=False, header=False)
    lens3_df.to_csv(lens3_filename, index=False, header=False)

    print(f"VPS data for {method_name} saved to {lens2_filename} and {lens3_filename}")
...
if __name__ == '__main__':
    ...
    save_vps_to_csv(lens2, lens3, method_name='HoGRC')  # 假设方法是RC
